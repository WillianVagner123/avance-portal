...
// tenta achar start/end em vários formatos
      ...
      >
        <div className="p-5">
          ...
          <div className="mt-4 space-y-2">
            ...
            >
              {googleLinked ? "Google vinculado ✓" : "Vincular ao Google Calendar"}
            </button>

            <div className="text-xs text-slate-500">
              (O vínculo é persistente quando seu backend salva refresh_token. Aqui só chama a rota.)
            </div>
          </div>
        </div>
      </div>
...
export default function CalendarPretty({ mode, title, subtitle }: Props) {
  ...
  // ✅ cache por janela (pra não duplicar)
  const [chunks, setChunks] = useState<Record<string, KonsistItem[]>>({});

  // ✅ google linked persistente (lido do backend)
    ...
    try {
      for (let i = 0; i <= totalDias; i += step) {
        ...
        try {
          ...
          // não quebra por falha em 1 chunk
        }
      }
    } finally {
      setLoading(false);
  ...
  async function prefetch30Dias() {
    ...
    for (let i = 0; i < totalDias; i += step) {
      const di = toISODate(addDays(start, i));
      const df = toISODate(addDays(start, Math.min(i + step, totalDias)));
 ...
 const events = useMemo(() => {
  ...
  for (let idx = 0; idx < filtered.length; idx++) {
    ...
    // ✅ DEDUPE: mesmo paciente + profissional + horário
  ...
  function handleLinkGoogle() {
    // ✅ botão “sempre” = você precisa de refresh_token salvo no backend
      ...
      <div className="min-h-[calc(100vh-1px)] w-full bg-slate-950 text-slate-100">
        <div className="mx-auto max-w-[1600px] px-3 py-6">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              <h1 className="text-xl font-black">{title || (mode === "admin" ? "Calendário — Admin" : "Meu Calendário")}</h1>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                title={googleLinked ? "Google já vinculado" : "Vincular Google Calendar"}
              >
                {googleLinked ? "Google vinculado ✓" : "Vincular Google"}
              </button>
                ...
                <div className="rounded-[28px] bg-slate-950/35 border border-white/10 p-4">
                      ...
                      >
                        ...
                        <div className="leading-tight">
                          ...
                          <div className="text-xs text-slate-400">
                            {k === "agenda" ? "Visão do dia" : k === "filtros" ? "Profissional / status" : k === "status" ? "Cores por status" : "Ações rápidas"}
                          </div>
                        </div>
                      </button>
                    ))}
                  ...
                  {tab === "status" && (
                    <div className="mt-3 rounded-2xl border border-white/10 bg-slate-950/40 p-3">
                      ...
                      {["Confirmado", "Agendado", "Não Confirmado", "Desmarcado", "Atendido pelo Medico", "Bloqueado"].map((s) => (
                  ...
                  {tab === "links" && (
                    <div className="mt-3 rounded-2xl border border-white/10 bg-slate-950/40 p-3 space-y-2">
                      ...
                      >
                        {googleLinked ? "Re-vincular (se necessário)" : "Vincular agora"}
                      </button>
                    </div>
                  )}
                </div>
